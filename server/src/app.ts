  ...
  ws.on('message', (data) => {
    try {
      ...
      if (data instanceof Buffer || data instanceof ArrayBuffer) {
        ...
        // 如果收到非二进制数据，记录警告并忽略
        Logger.warn('收到非二进制数据，已忽略', { 
          clientId: session.id.substring(0, 12) + '...',
          dataType: typeof data,
          dataLength: typeof data === 'string' ? (data as string).length  : 'N/A'
        });
      }
...
function handleAudioData(session: any, data: Buffer | ArrayBuffer): void {
  ...
  // 详细日志（每20个数据块打印一次）
  if (session.audioStats.totalChunks % 20 === 0) {
    ...
    Logger.info('🎵 音频数据统计', {
      ...
      运行时间: `${Math.round((Date.now() - session.connectedAt.getTime()) / 1000)} 秒`
    });
  }
...
function saveAudioChunk(session: any, chunk: Buffer): void {
  try {
    ...
    const clientDir = path.join(recordingsDir, session.id.substring(0, 8), dateStr);
...
process.on('SIGINT', () => {
  ...
  const totalChunks = allClients.reduce((sum, c) => sum + c.audioStats.totalChunks, 0);
  const totalBytes = allClients.reduce((sum, c) => sum + c.audioStats.totalBytes, 0);
